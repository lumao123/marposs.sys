...
<head>
    ...
    <style>
        header,footer{width: 100%; height: 40px; background: #1976D2;}
        ...
        ul{
            list-style: none; width: 500px; height: auto; margin: 100px 100px 0 0;
        ...
        ul li input{
            ...
            margin: 0 0; display: inline-block; padding-left: 10px;;
            border: 1px solid #ccc; border-radius: 2px; font-size: 16px;
        }
        ul li span{
            width: 404px; height: 44px; line-height: 44px; color: #fff;
            margin: 0 0; border-radius: 2px; font-size: 16px;
        ...
        .toLogin{float: right; margin: 20px 60px;}
    </style>
</head>
<body>
<section>
    ...
    <div class="right">
        <ul>
            ...
            <li><a class="toLogin" href="<%=request.getContextPath()%>/user/toLogin">Already have an account，to login?</a></li>
        </ul>
    </div>
</section>
</body>
<script>
    ...
    $('#rgBtn').bind('click', function(){
        ...
        if(!comfirmPwd) {setMsg('Confirm password is empty！'); return}
        if(comfirmPwd != password) {setMsg('Confirm password is not same to password！'); return}
        $.ajax({
    ...
    $('input').on('focus', function(){setMsg('')});
    function setMsg(msg){
        $('.msg').empty().text(msg);
    }
</script>
